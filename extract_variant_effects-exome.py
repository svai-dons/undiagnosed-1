...
variants = pd.read_csv('results/exome-variants-snpeff-grch37.75-gencode.bed', sep='\t',
                       index_col=None)
...
outfile_ann = 'results/exome-variants-snpeff-grch37.75-gencode-annotations.vcf'
with open(outfile_ann, 'w+') as fh:
    first = True
    for kk,vv in tqdm(variants.iterrows()):

        tmp_bed = vv.loc['CHROM':'RESULT']
        tmp_bed = tmp_bed.drop(['INFO', 'FORMAT'])
        ...
        if first:
            tmp_ann.columns = [('#'+x if ii==0 else x) for ii, x in enumerate(tmp_ann.columns )]
